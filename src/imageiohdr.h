...
template<typename T>
inline bool hdrWriteHeader( FILE* in_F, int32_t in_Width, int32_t in_Height, T in_Gamma, T in_Exposure )
{
...
{
  bool operator()( FILE* in_F, T* in_Data, int32_t in_NumPixels ) { return true; }
};

template<typename T>
struct hdrWritePixels<T, 1>
{
  bool operator()( FILE* in_F, T* in_Data, int32_t in_NumPixels )
  {
...
inline bool hdrWriteBytesRLE( FILE* in_F, unsigned char* in_Bytes, int32_t in_NumBytes )
{
...
{
  bool operator()( FILE* in_F, T* in_Pixels, int32_t in_LineWidth, int32_t in_NumLines ) { return true; }
};

template<typename T>
struct hdrWritePixelsRLE<T, 1>
{
  bool operator()( FILE* in_F, T* in_Pixels, int32_t in_LineWidth, int32_t in_NumLines )
  {
    unsigned char rgbe[4];
    ...
    while( in_NumLines-- > 0 )
    {
      ...
      for( int32_t i=0; i<in_LineWidth; i++ )
      {
      ...
      {
        if( false == hdrWriteBytesRLE( in_F, &buffer[ i * in_LineWidth ], in_LineWidth ) )
        {
          free( buffer );
          return false;
        }
...
template<typename T>
bool hdrReadHeader( FILE* in_F, int32_t* out_Width, int32_t* out_Height, T* out_Gamma, T* out_Exposure )
{
  char buf[128];
  float tempf;
...
{
  bool operator()( FILE* in_F, T* out_Data, int32_t in_NumPixels ) { return true; }
};

template<typename T>
struct hdrReadPixels<T, 1>
{
  bool operator()( FILE* in_F, T* out_Data, int32_t in_NumPixels )
  {
    unsigned char rgbe[4];
    T data[3];
...
{
  bool operator()( FILE* in_F, T* in_Data, int32_t in_ScanlineWidth, int32_t in_NumScanlines ) { return true; }
};

template<typename T>
struct hdrReadPixelsRLE<T, 1>
{
  bool operator()( FILE* in_F, T* in_Data, int32_t in_ScanlineWidth, int32_t in_NumScanlines )
  {
    ...
    while( in_NumScanlines > 0 )
    {
      ...
      for( int32_t i=0; i<in_ScanlineWidth; i++ )
      {
...
template<typename T, int N>
inline bool loadImageHdr( const char* in_Filename, Image<T, N>& io_Image )
{
  FILE* f;
#ifdef _MSC_VER
  ...
  if( false == hdrReadHeader<T>( f, &width, &height, &gamma, &exposure ) )
  {
    fclose(f);
    return false;
  }
  
  io_Image.resize( width, height );
  
  //std::cout << io_Image.width << "*" << io_Image.height << "*" << N << std::endl;  
  if( false == hdrReadPixelsRLE<T, N>()( f, io_Image.getDataPointer(), width, height ) )
  {
    fclose(f);
    return false;
  }
  else
  {
    ...
    //for( int i=0; i<io_Image.width*io_Image.height*N; i++ )
    //  std::cout << io_Image.ptr[i] << ", ";
    //std::cout << std::endl;
    return true;
  }
}
...
template<typename T, int N>
inline bool saveImageHdr( const char* in_Filename, const Image<T, N>& in_Image )
{
  FILE* f;
#ifdef _MSC_VER
  ...
  if( f == NULL ) return false;
#endif
  hdrWriteHeader<T>( f, in_Image.getWidth(), in_Image.getHeight(), T(1.0), T(1.0) );
  //hdrWritePixels(f, in_PixelColors3, in_Width*in_Height);
  hdrWritePixelsRLE<T, N>()( f, in_Image.getConstDataPointer(), in_Image.getWidth(), in_Image.getHeight() );
  fclose(f);
  return true;
}
